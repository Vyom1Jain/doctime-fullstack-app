...
const BillingPage = () => {
  ...
  useEffect(() => {
    const fetchBilling = async () => {
      try {
        const res = await axios.get(`/api/billing/patient/${user.userId}`)
  ...
  return (
    <div className="min-h-screen bg-slate-50 px-4 pt-4 pb-20 space-y-4">
      <header>
        ...
        <p className="text-sm text-slate-500">View your payments and consultation history.</p>
      </header>
      <section className="card space-y-1">
        <p className="text-xs text-slate-500 font-medium uppercase">Total spent</p>
      ...
      <main className="space-y-3">
        ...
        {transactions.map((tx) => (
          <div key={tx.id} className="card flex items-center justify-between text-sm">
            <div>
              <p className="font-semibold text-slate-900">{tx.description || 'Consultation payment'}</p>
              <p className="text-xs text-slate-500">
                {new Date(tx.createdAt).toLocaleString()}
              </p>
            </div>
            <div className="text-right">
              <p className="font-semibold text-slate-900">₹ {Number(tx.amount || 0).toFixed(2)}</p>
